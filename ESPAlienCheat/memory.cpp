...
bool kernel::driver::init()
{
	if (!pHookFunc)
	{
		...
		pHookFunc = (pfunc_hk_t)GetProcAddress(hDll, skCrypt("NtUserInitializeInputDeviceInjection"));
		if (!pHookFunc)
		{
...
NTSTATUS kernel::driver::setwindowaffinity(HWND window_handle, uint32_t value)
{
	fptr_data::kernel_com com{};
	com.request_key = 10;
